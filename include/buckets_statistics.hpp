...
struct buckets_statistics {
    ...
    void add_bucket_size(uint64_t bucket_size) {
        if (bucket_size < MAX_BUCKET_SIZE + 1) {
            ...
            m_max_buckets_per_size = std::max(m_max_buckets_per_size, m_bucket_sizes[bucket_size]);
        }
    ...
    uint64_t max_buckets_per_size() const { return m_max_buckets_per_size; }
    ...
    void operator+=(buckets_statistics const& rhs) {
        ...
        if (rhs.max_buckets_per_size() > m_max_buckets_per_size) {
            m_max_buckets_per_size = rhs.max_buckets_per_size();
        }
...
private:
    ...
    uint64_t m_max_buckets_per_size;
